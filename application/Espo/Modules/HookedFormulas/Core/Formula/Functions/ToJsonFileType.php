...
class ToJsonFileType extends \Espo\Core\Formula\Functions\Base
{
    public function process(\StdClass $item)
    {
        ...
        if (!is_array($item->value)) {
            throw new Error('Value for \'toJsonFile\' item is not array.');
        }

        if (count($item->value) != 2) {
            throw new Error('\'toJsonFile\' needs <filename> and <variable>.');
        }

        $filename = $this->evaluate($item->value[0]);
        $obj = $this->evaluate($item->value[1]);
